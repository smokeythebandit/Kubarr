...
impl K8sClient {
    ...
    pub async fn get_service_endpoints(
        ...
        // Read Kubarr proxy annotations from service metadata.
        let annotations = service.metadata.annotations.as_ref();
        let base_path = annotations
            .and_then(|annotations| annotations.get("kubarr.io/base-path"))
            .cloned();
        let landing_path = annotations
            .and_then(|annotations| annotations.get("kubarr.io/landing-path"))
            .cloned();
