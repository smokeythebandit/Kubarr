...
//! - `GET /api/system/version` — requires authentication
...
#[tokio::test]
async fn test_version_endpoint_requires_auth() {
    let state = build_test_app_state().await;
    let app = create_router(state);
...
#[tokio::test]
async fn test_version_endpoint_unauthenticated_does_not_return_version() {
    let state = build_test_app_state().await;
    let app = create_router(state);
...
#[tokio::test]
async fn test_openapi_json_requires_auth() {
    let state = build_test_app_state().await;
    let app = create_router(state);
...
#[tokio::test]
async fn test_openapi_json_unauthenticated_does_not_return_spec() {
    let state = build_test_app_state().await;
    let app = create_router(state);
...
#[tokio::test]
async fn test_swagger_ui_unauthenticated_does_not_return_html() {
    let state = build_test_app_state().await;
    let app = create_router(state);
