...
// ============================================================================
// Gateway-owned Route Tests
// ============================================================================

#[tokio::test]
async fn test_api_does_not_serve_frontend_spa_routes() {
    let state = create_test_state().await;

    // Frontend SPA routes are served by the gateway/frontend Service, not the API.
    ...
    for route in spa_routes {
        ...
        assert_ne!(
            status,
            StatusCode::OK,
            "API must not serve frontend SPA route {}",
            route
        );
    }
}
...
#[tokio::test]
async fn test_api_does_not_proxy_app_routes() {
    let state = create_test_state().await;

    // App routes are authenticated and proxied by the gateway, not the API.
    ...
    for route in app_routes {
        ...
        assert_ne!(
            status,
            StatusCode::OK,
            "API must not proxy app route {} (got {})",
            route,
            status
        );
    }
...
async fn test_auth_architecture_summary() {
    ...
    //    - Frontend and app paths are handled by the gateway, not the API

    // 2. PROTECTED ROUTES:
    //    - All /api/* routes except public health endpoints
