...
async fn test_setup_status_after_server_config_shows_configured() {
    ...
    assert_eq!(
        json["storage_configured"], false,
        "storage_configured must remain false until storage is configured. Body: {}",
        body
    );
}
...
async fn test_get_server_config_returns_config_after_configure() {
    ...
    assert_eq!(
        json["storage_path"], "/data",
        "Storage path is fixed by the media storage contract"
    );
}
...
#[tokio::test]
async fn test_configure_server_response_has_name_and_fixed_storage_path() {
    let state = build_test_app_state().await;
...
async fn test_configure_server_missing_fields_returns_422() {
    ...
    // Missing name
    let body = serde_json::json!({"storage_path": "/tmp"}).to_string();
