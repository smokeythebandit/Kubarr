...
impl BootstrapService {
    ...
    async fn ensure_storage_ready(&self) -> Result<()> {
        let db_guard = self.db.read().await;
        let k8s_guard = self.k8s.read().await;
        let ready = storage_config::storage_ready(db_guard.as_ref(), k8s_guard.as_ref()).await?;
        if !ready {
    ...
    async fn ensure_component_storage(&self, namespace: &str) -> Result<()> {
        let db_guard = self.db.read().await;
        let k8s_guard = self.k8s.read().await;
        let k8s = k8s_guard.as_ref().ok_or_else(|| {
            AppError::ServiceUnavailable("Kubernetes client not available".to_string())
        ...
        let config = if let Some(db) = db_guard.as_ref() {
            storage_config::get_storage_config_from_db(db)
                .await?
                .map(|(config, _)| config)
        } else {
            storage_config::load_pre_db_secret(k8s).await?
        }
        .ok_or_else(|| {
            AppError::BadRequest(
                "Storage must be configured and validated before bootstrap can start".to_string(),
        ...
        if !config.validated() {
            return Err(AppError::BadRequest(
                "Storage must be validated before bootstrap can start".to_string(),
            ));
        }

        storage_config::ensure_media_pvc_for_namespace(k8s, namespace, &config).await
    }
    ...
    async fn install_postgresql(&self) -> Result<()> {
        ...
        self.ensure_component_storage("postgresql").await?;
            ...
            match db::connect_with_url(&database_url).await {
                Ok(conn) => {
                    ...
                    if let Some(k8s) = k8s_guard.as_ref() {
                        match storage_config::migrate_pre_db_secret_to_db(&conn, k8s).await {
                            Ok(Some(_)) => {
                                tracing::info!("Migrated pre-database storage config into DB")
                            }
                            Ok(None) => {}
                            Err(e) => {
                                tracing::warn!("Failed to migrate storage config into DB: {}", e)
                            }
                        }
                    }
    ...
    async fn install_component(&self, component: &str, display_name: &str) -> Result<()> {
        ...
        self.ensure_component_storage(component).await?;
