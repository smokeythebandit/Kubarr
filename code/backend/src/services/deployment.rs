...
impl<'a> DeploymentManager<'a> {
    ...
    pub async fn deploy_app(
        ...
        if let Some(storage) = storage_config {
            ...
            storage_config::ensure_media_pvc_for_namespace(self.k8s, namespace, storage).await?;
            set_args.extend(media_storage_helm_values());
        }
...
pub fn media_storage_helm_values() -> Vec<String> {
    vec![
        format!(
