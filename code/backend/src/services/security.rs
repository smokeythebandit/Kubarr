...
#[cfg(test)]
static TEST_JWT_KEY_PAIR: Lazy<(String, String)> =
    Lazy::new(|| generate_rsa_key_pair().expect("generate test JWT key pair"));
...
pub async fn init_jwt_keys(db: &sea_orm::DatabaseConnection) -> Result<()> {
    ...
    let (private_pem, public_pem) = match (private_setting, public_setting) {
        ...
        _ => {
            ...
            let (private_pem, public_pem) = generate_jwt_key_pair_for_init()?;
...
fn generate_jwt_key_pair_for_init() -> Result<(String, String)> {
    #[cfg(test)]
    {
        Ok(TEST_JWT_KEY_PAIR.clone())
    }
...
mod tests {
    ...
    static JWT_TEST_LOCK: Lazy<tokio::sync::Mutex<()>> = Lazy::new(|| tokio::sync::Mutex::new(()));
