...
- Audit records capture implemented authentication/session, user/role/invite/settings, VPN, and queued app-operation events; worker terminal outcomes use a transactional outbox. Audit reads and manual clearing require `audit.view` and `audit.manage` respectively. Retention runs every 24 hours (default 90 days, configurable with `KUBARR_AUDIT_RETENTION_DAYS` from 1–3650); automatic and manual cleanup record their audit event atomically. Coverage has boundaries: peer IP may identify the gateway rather than browser, app-access events record requests rather than confirmed rendering, and interrupted worker operations become indeterminate after 15 minutes without automatic rerun. Audit events and UI counts are not comprehensive proof of external outcomes or 2FA adoption.
